...
searchIssueForm.addEventListener('submit', function (e) {
  ...
  let descriptionValue =
    searchIssueForm.querySelector('input[name="des"]').value.toLowerCase();

  //add issue to searched issues array
  searchIssues.map((el) => {
    
    if (el.title.toLowerCase() == titleValue || el.description.toLowerCase() == descriptionValue || 
        ( !(!titleValue) && el.title.toLowerCase().includes(titleValue)) || ( !(!descriptionValue) && el.description.toLowerCase().includes(descriptionValue)) ) {
